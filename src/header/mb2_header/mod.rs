...
use RelocatableHeaderTag;
use {AddressHeaderTag, InformationRequestHeaderTag};
use {ConsoleHeaderTag, EntryHeaderTag};
use {EfiBootServiceHeaderTag, FramebufferHeaderTag};
use {EndHeaderTag, HeaderTagType};
use {EntryEfi32HeaderTag, EntryEfi64HeaderTag};
use {HeaderTag, HeaderTagISA};
...
impl<'a> Multiboot2Header<'a> {
    pub(super) fn new(inner: &'a Multiboot2HeaderInner) -> Self {
        Self { inner }
    }
...
#[derive(Copy, Clone)]
#[repr(C, packed)]
pub(crate) struct Multiboot2HeaderInner {
