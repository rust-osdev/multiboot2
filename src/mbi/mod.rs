...
pub use self::framebuffer::{FramebufferColor, FramebufferField, FramebufferTag, FramebufferType};
pub use self::memory_map::{
    EFIMemoryAreaType, EFIMemoryDesc, EFIMemoryMapTag, MemoryArea, MemoryAreaIter, MemoryAreaType,
    MemoryMapTag,
};
pub use self::module::{ModuleIter, ModuleTag};
pub use self::rsdp::{
    EFIImageHandle32, EFIImageHandle64, EFISdt32, EFISdt64, ImageLoadPhysAddr, RsdpV1Tag, RsdpV2Tag,
};
pub use self::tags::{MbiTag, MbiTagType};
pub use self::vbe_info::{
...
impl BootInformation {
    ...
    pub fn elf_sections_tag(&self) -> Option<ElfSectionsTag> {
        self.get_tag(MbiTagType::ElfSections)
            .map(|tag| unsafe { elf_sections::elf_sections_tag(tag, self.offset) })
    }

    /// Search for the Memory map tag.
    pub fn memory_map_tag<'a>(&'a self) -> Option<&'a MemoryMapTag> {
        self.get_tag(MbiTagType::Mmap)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const MemoryMapTag) })
    }
    ...
    pub fn boot_loader_name_tag<'a>(&'a self) -> Option<&'a BootLoaderNameTag> {
        self.get_tag(MbiTagType::BootLoaderName)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const BootLoaderNameTag) })
    }

    /// Search for the Command line tag.
    pub fn command_line_tag<'a>(&'a self) -> Option<&'a CommandLineTag> {
        self.get_tag(MbiTagType::Cmdline)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const CommandLineTag) })
    }

    /// Search for the VBE framebuffer tag.
    pub fn framebuffer_tag<'a>(&'a self) -> Option<FramebufferTag<'a>> {
        self.get_tag(MbiTagType::Framebuffer)
            .map(|tag| framebuffer::framebuffer_tag(tag))
    }

    /// Search for the EFI 32-bit SDT tag.
    pub fn efi_sdt_32_tag<'a>(&self) -> Option<&'a EFISdt32> {
        self.get_tag(MbiTagType::Efi32)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const EFISdt32) })
    }

    /// Search for the EFI 64-bit SDT tag.
    pub fn efi_sdt_64_tag<'a>(&self) -> Option<&'a EFISdt64> {
        self.get_tag(MbiTagType::Efi64)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const EFISdt64) })
    }

    /// Search for the (ACPI 1.0) RSDP tag.
    pub fn rsdp_v1_tag<'a>(&self) -> Option<&'a RsdpV1Tag> {
        self.get_tag(MbiTagType::AcpiV1)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const RsdpV1Tag) })
    }

    /// Search for the (ACPI 2.0 or later) RSDP tag.
    pub fn rsdp_v2_tag<'a>(&'a self) -> Option<&'a RsdpV2Tag> {
        self.get_tag(MbiTagType::AcpiV2)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const RsdpV2Tag) })
    }
    ...
    pub fn efi_memory_map_tag<'a>(&'a self) -> Option<&'a EFIMemoryMapTag> {
        ...
        match self.get_tag(MbiTagType::EfiBs) {
            Some(_tag) => None,
            None => self
                .get_tag(MbiTagType::EfiMmap)
                .map(|tag| unsafe { &*(tag as *const MbiTag as *const EFIMemoryMapTag) }),
        }
    }

    /// Search for the EFI 32-bit image handle pointer.
    pub fn efi_32_ih<'a>(&'a self) -> Option<&'a EFIImageHandle32> {
        self.get_tag(MbiTagType::Efi32Ih)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const EFIImageHandle32) })
    }

    /// Search for the EFI 64-bit image handle pointer.
    pub fn efi_64_ih<'a>(&'a self) -> Option<&'a EFIImageHandle64> {
        self.get_tag(MbiTagType::Efi64Ih)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const EFIImageHandle64) })
    }

    /// Search for the Image Load Base Physical Address.
    pub fn load_base_addr<'a>(&'a self) -> Option<&'a ImageLoadPhysAddr> {
        self.get_tag(MbiTagType::LoadBaseAddr)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const ImageLoadPhysAddr) })
    }

    /// Search for the VBE information tag.
    pub fn vbe_info_tag(&self) -> Option<&'static VBEInfoTag> {
        self.get_tag(MbiTagType::Vbe)
            .map(|tag| unsafe { &*(tag as *const MbiTag as *const VBEInfoTag) })
    }
    ...
    fn get_tag<'a>(&'a self, typ: MbiTagType) -> Option<&'a MbiTag> {
        self.tags().find(|tag| tag.typ == typ)
    }

    fn tags(&self) -> MbiTagIter {
        MbiTagIter::new(unsafe { self.inner.offset(1) } as *const _)
    }
}

impl BootInformationInner {
    fn has_valid_end_tag(&self) -> bool {
        const END_TAG: MbiTag = MbiTag {
            typ: MbiTagType::End,
            size: 8,
        };
        ...
        let end_tag = unsafe { &*(end_tag_addr as *const MbiTag) };
...
mod tests {
    ...
    use {VBECapabilities, VBEMemoryModel};
    use {VBEDirectColorAttributes, VBEField};
    use {VBEModeAttributes, VBEWindowAttributes};
    ...
    fn framebuffer_tag_rgb() {
        ...
        assert_eq!(addr + bytes.0.len(), bi.end_address());
        assert_eq!(bytes.0.len(), bi.total_size());
        use super::framebuffer::{FramebufferField, FramebufferTag, FramebufferType};
        assert_eq!(
            bi.framebuffer_tag(),
            Some(FramebufferTag {
                address: 4244635648,
    ...
    fn framebuffer_tag_indexed() {
        ...
        assert_eq!(addr + bytes.0.len(), bi.end_address());
        assert_eq!(bytes.0.len(), bi.total_size());
        use super::framebuffer::{FramebufferColor, FramebufferType};
    ...
    #[test]
    fn efi_memory_map() {
        use super::memory_map::EFIMemoryAreaType;
        #[repr(C, align(8))]
        struct Bytes([u8; 72]);
