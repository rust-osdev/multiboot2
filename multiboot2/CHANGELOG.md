...
- **Breaking** Changed `multiboot2::elf_sections` to use
  the [elf](https://docs.rs/elf/latest/elf/) crate
  - `ElfSectionsTag::sections()` now returns an iterator over
    `elf::section::SectionHeader`.
  - `ElfSection` has been removed and replaced with
    `elf::section::SectionHeader`.
- Added `ElfSectionExt` trait to replace functionality for
  `ElfSection::flags()`,
